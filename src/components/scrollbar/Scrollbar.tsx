import { defineComponent, onMounted, onUpdated, ref, reactive, onUnmounted, Transition } from 'vue'
...
export default defineComponent({
  ...
  setup (props, { slots }) {
    ...
    return () => (
      <div class="the-scrollbar" ref={el} onMouseenter={updateThumbStyle} onMouseleave={onLeave}>
        <div class={[props.vertical ? 'the-scroll-y' : 'the-scroll-x']} style={wrapStyle} ref={wrap}>
          {/* https://juejin.cn/post/6891585729843953671 */}
          {slots && slots.default?.()}
        </div>
        <Transition name="fade">
          <button class="the-scroll-thumb" title="滚动条-摁住拖拽" ref={thumb} style={thumbStyle}></button>
        </Transition>
      </div>
    )
  }
